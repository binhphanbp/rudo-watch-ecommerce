...
async function getFeaturedProducts() {
  const response = await fetch(
    'http://localhost/rudo-watch-ecommerce/backend/api/v1/products/featured?limit=8'
  );
...
async function getLatestProducts() {
  const response = await fetch(
    'http://localhost/rudo-watch-ecommerce/backend/api/v1/products/latest?limit=8'
  );
  const result = await response.json();
  return result.data;
}
...
async function getProducts(page = 1, limit = 12) {
  const response = await fetch(
    `http://localhost/rudo-watch-ecommerce/backend/api/v1/products?page=${page}&limit=${limit}&status=1`
  );
  const result = await response.json();
