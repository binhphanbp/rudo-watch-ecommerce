...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim()) {
      Swal.fire({ icon: 'warning', title: 'Tên danh mục không được trống' });
      return;
    }
    setSubmitting(true);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative max-w-sm">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm danh mục..." className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50">
                <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Danh mục</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
              {loading ? (
                ...
                filtered.map((cat) => (
                  <tr key={cat._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        {cat.image ? (
                          <Image src={getImageUrl(cat.image)} alt={cat.name} width={40} height={40} className="w-10 h-10 rounded-lg object-cover bg-gray-100 dark:bg-slate-700" />
                        ) : (
                    ...
                    <td className="px-5 py-4 text-sm text-gray-500 hidden md:table-cell">
                      <code className="px-2 py-0.5 bg-gray-100 dark:bg-slate-700 rounded text-xs">{cat.slug}</code>
                    </td>
                    <td className="px-5 py-4 text-sm text-gray-500 hidden lg:table-cell">{formatDateTime(cat.created_at)}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(cat)} className="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Sửa">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(cat._id, cat.name)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Xoá">
                ...
                <tr>
                  <td colSpan={4} className="px-5 py-12 text-center">
                    ...
                    <p className="text-gray-500 text-sm">{search ? 'Không tìm thấy danh mục' : 'Chưa có danh mục nào'}</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{editing ? 'Sửa danh mục' : 'Thêm danh mục'}</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 block">Tên danh mục <span className="text-red-500">*</span></label>
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ví dụ: Đồng hồ nam" className="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl text-sm bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500" autoFocus />
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 block">Hình ảnh (URL)</label>
                <input type="text" value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} placeholder="https://... hoặc /images/..." className="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl text-sm bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
