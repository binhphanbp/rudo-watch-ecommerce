...
function setFormLoading(loading) {
	...
	const inputs = voucherForm.querySelectorAll('input, select, button[type="submit"]');
	inputs.forEach(input => {
		if (input.type === 'submit') {
			if (loading) {
				...
				input.innerHTML = '<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span> Đang xử lý...';
			} else {
	...
	voucherTableBody.innerHTML = vouchers
		.map(
			(voucher, index) => {
				...
				const now = new Date();
				const expiredDateOnly = new Date(expiredDate.getFullYear(), expiredDate.getMonth(), expiredDate.getDate());
				const nowDateOnly = new Date(now.getFullYear(), now.getMonth(), now.getDate());
				const isExpired = expiredDateOnly < nowDateOnly;
				const daysUntilExpiry = Math.ceil((expiredDateOnly - nowDateOnly) / (1000 * 60 * 60 * 24));
				...
				if (voucher.type === "percent") {
					if (voucher.discount !== null && voucher.discount !== undefined) {
						...
						discountValue = '<span class="text-muted">0% <i class="ti ti-alert-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Chưa có giá trị giảm giá"></i></span>';
					}
				} else {
					if (voucher.discount !== null && voucher.discount !== undefined) {
						discountValue = formatCurrency(voucher.discount);
					} else {
						discountValue = '<span class="text-muted">0 đ <i class="ti ti-alert-circle ms-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Chưa có giá trị giảm giá"></i></span>';
					}
				}

				const amountStr = voucher.amount !== null && voucher.amount !== undefined ? String(voucher.amount) : "";
				const amountNum = amountStr ? parseInt(amountStr) : 0;
				const isUsable = amountStr !== "" && !isNaN(amountNum) && amountNum > 0;

				let alertIcon = "";
				if (!isUsable) {
					alertIcon = '<i class="ti ti-alert-circle ms-1 text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Không thể sử dụng"></i>';
				} else if (isExpired) {
					alertIcon = '<i class="ti ti-alert-circle ms-1 text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Đã hết hạn"></i>';
				} else if (isExpiringSoon) {
					alertIcon = '<i class="ti ti-alert-circle ms-1 text-warning" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Sắp hết hạn"></i>';
				}
				...
				return `
						...
						<td>
							<span class="badge ${voucher.type === "percent"
						...
						<td>
							${discountValue.includes('<span') ? discountValue : `<span class="fw-semibold">${discountValue}</span>`}
						</td>
						<td>
							<span class="${isExpired ? "text-danger" : isExpiringSoon ? "text-warning" : !isUsable ? "text-danger" : ""}">
						...
						const amountStr = voucher.amount !== null && voucher.amount !== undefined ? String(voucher.amount) : "";
						const amountNum = amountStr ? parseInt(amountStr) : 0;
						return amountStr !== "" && !isNaN(amountNum) && amountNum > 0
						...
						<td>
							...
							<button class="btn btn-sm btn-outline-primary" onclick="editVoucher(${voucher.id
					})" title="Sửa">
								<i class="ti ti-edit"></i>
							</button>
							<button class="btn btn-sm btn-outline-danger" onclick="confirmDelete(${voucher.id
...
function handleFormSubmit(e) {
	...
	const data = {
		...
		amount: usageAmountNum, // Gửi như số
	};
...
window.editVoucher = function (id) {
	...
	if (voucher.type === "percent") {
		...
		voucherMoneyDiscountInput.value = voucher.discount || "";
	}

	voucherAmountInput.value = voucher.amount !== null && voucher.amount !== undefined ? String(voucher.amount) : "";
