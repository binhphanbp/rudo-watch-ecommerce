...
const handleFormSubmit = async (e) => {
	...
	try {
		// 1. Create product (không bao gồm variants)
		...
		if (failedVariants.length > 0) {
			...
			Swal.close();
			showErrorDialog(`Tạo sản phẩm thành công nhưng có lỗi khi lưu variants: ${errorMessages}`, 'Cảnh báo');
		} else {
			Swal.close();
		...
		showErrorDialog(errorMsg, 'Lỗi tạo sản phẩm');
	}
